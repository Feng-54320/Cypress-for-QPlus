import AuxiliaryPlanPage from "../../page/mysql_page/auxiliary/auxiliary_plan_page.cy";

describe("module：MySQL辅助计划", () => {
  let auxiliaryPlanPage;

  //用例前置：读取qplus的url和登陆用例的测试数据
  beforeEach(() => {
    cy.loginCommand();
    cy.fixture("/locator/mysql/auxiliary/elements.json").then((elements) => {
  ...
  it("case 1： 验证创建时间点快照库", () => {
    ...
    //12. 增加数据存储, 参数同上，int类型, 参数为几就是增加几
